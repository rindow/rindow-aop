...
class Get extends AbstractDesignator
{
    protected $regex;
    ...
    public function setPattern($pattern,$location=null)
    {
        if(!preg_match('/^[a-zA-Z0-9_\\\\*]+::\$\\(?[a-zA-Z0-9_\\*|]+\\)?$/', $pattern))
            throw new Exception\DomainException('a "get" pointcut must contain a class and property with wildcard."'.$pattern.'":'.$location);
    ...
    {
        if($joinpoint->getAction()!=JoinPointInterface::ACTION_GET)
            return false;
